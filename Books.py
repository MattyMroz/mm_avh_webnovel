...
def erasing_words():
    ...
    with open('0.txt', 'w', encoding='utf8') as f:
        for line in lines:
            ...
            # Usuń niepotrzebne znaki nie będące częścią ASCII, ale zostaw polskie znaki i znaki interpunkcyjne
            line = re.sub(
                r'[^\x00-\x7FąęółśżźćńĄĘÓŁŚŻŹĆŃ.,;:!?\'\"…]+', '', line)
            # Usuń samotne kropki
            line = re.sub(r'(?<=\s)\.(?=\s)|^\.\s|\s\.$', '', line)
            # Usuń puste linie i spacje na początku oraz na końcu linii
            line = line.strip()
            if line:
